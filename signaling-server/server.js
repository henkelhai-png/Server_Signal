...
const GROUP_MESSAGES_FILE = process.env.GROUP_MESSAGES_FILE || path.join(DATA_DIR, "group_messages.json");
const UPLOADS_DIR = process.env.UPLOADS_DIR || path.join(__dirname, "uploads");
const MAX_GROUP_FILE_BYTES = Number(process.env.MAX_GROUP_FILE_BYTES || 25 * 1024 * 1024);
const SESSION_TTL_MS = Number(process.env.SESSION_TTL_MS || 1000 * 60 * 60 * 24 * 7);
const MAX_MESSAGE_BYTES = Number(process.env.MAX_MESSAGE_BYTES || 50 * 1024 * 1024);
 ...
 * 5) сохранение истории групповых текстовых сообщений и файлов.
 */
...
if (!fs.existsSync(GROUP_MESSAGES_FILE)) {
  fs.writeFileSync(GROUP_MESSAGES_FILE, JSON.stringify({}, null, 2), "utf8");
}
...
const server = http.createServer((req, res) => {
  ...
  if (req.method === "GET" && (url.pathname.startsWith("/uploads/") || url.pathname.startsWith("/download/"))) {
    const isDownload = url.pathname.startsWith("/download/");
    const requestedName = decodeURIComponent(url.pathname.replace(isDownload ? "/download/" : "/uploads/", ""));
    ...
    res.writeHead(200, {
      "content-type": isDownload ? "application/octet-stream" : (contentTypes[ext] || "application/octet-stream"),
      "cache-control": "public, max-age=31536000",
      "content-disposition": `${isDownload ? "attachment" : "inline"}; filename*=UTF-8''${encodeURIComponent(safeName)}`,
    });
  ...
  if (url.pathname === "/" || url.pathname === "/health") {
    const payload = JSON.stringify({
      ...
      totalStoredGroupMessages: Object.values(loadGroupMessages()).reduce((sum, items) => sum + (Array.isArray(items) ? items.length : 0), 0),
...
function loadGroupMessages() {
  try {
    ...
    const raw = fs.readFileSync(GROUP_MESSAGES_FILE, "utf8");
    if (!raw.trim()) {
      return {};
    }
    ...
    console.error("Cannot load group messages file:", error);
    return {};
  }
}

function saveGroupMessages(messages) {
  const tmpFile = `${GROUP_MESSAGES_FILE}.tmp`;
  fs.writeFileSync(tmpFile, JSON.stringify(messages, null, 2), "utf8");
  fs.renameSync(tmpFile, GROUP_MESSAGES_FILE);
}
...
function normalizeStoredMessage(message, usernameForOwnFlag) {
  return {
    ...message,
    isOwn: Boolean(usernameForOwnFlag && message.author === usernameForOwnFlag),
  };
}

function sendGroupHistoryToSocket(ws, groupId) {
  const messages = loadGroupMessages();
  const groupMessages = Array.isArray(messages[groupId]) ? messages[groupId] : [];

  safeSend(ws, {
    type: "group:messages",
    groupId,
    messages: groupMessages.map((message) => normalizeStoredMessage(message, ws.username)),
  });
}
...
function handleGroupMessagesGet(ws, data) {
  if (!requireAuthenticated(ws)) return;

  const groupId = String(data.groupId || "");
  if (!groupId || !isGroupMember(groupId, ws.username)) {
    safeSend(ws, { type: "error", message: "Нет доступа к истории этой группы" });
    return;
  }

  sendGroupHistoryToSocket(ws, groupId);
}

function handleGroupTextMessage(ws, data) {
  if (!requireAuthenticated(ws)) return;
  ...
  if (!groupId || !text) {
    safeSend(ws, { type: "error", message: "Пустое сообщение или группа не выбрана" });
    return;
  }

  if (!isGroupMember(groupId, ws.username)) {
    safeSend(ws, { type: "error", message: "Вы не являетесь участником этой группы" });
    return;
  }

  const messages = loadGroupMessages();
  if (!Array.isArray(messages[groupId])) {
    messages[groupId] = [];
  }
...
function sanitizeFileName(fileName) {
  return path.basename(String(fileName || "file")).replace(/[^a-zA-Zа-яА-ЯёЁ0-9_.-]/g, "_") || "file";
}

function handleGroupFileMessage(ws, data) {
  if (!requireAuthenticated(ws)) return;

  const groupId = String(data.groupId || "");
  const fileName = sanitizeFileName(data.fileName || "file");
  const mimeType = String(data.mimeType || "application/octet-stream");
  const fileBase64 = String(data.fileBase64 || "");
  const declaredFileSize = Number(data.fileSize || 0);

  if (!groupId || !fileBase64) {
    safeSend(ws, { type: "error", message: "Не хватает данных для сохранения файла" });
    return;
  }
  ...
  if (buffer.length > MAX_GROUP_FILE_BYTES) {
    safeSend(ws, { type: "error", message: `Файл слишком большой. Максимум: ${Math.round(MAX_GROUP_FILE_BYTES / 1024 / 1024)} МБ.` });
    return;
  }

  if (declaredFileSize && Math.abs(buffer.length - declaredFileSize) > 2) {
    console.warn(`[group:file] declared size mismatch for ${fileName}: declared=${declaredFileSize}, actual=${buffer.length}`);
  }

  const savedFileName = `${Date.now()}_${crypto.randomUUID()}_${fileName}`;
...
function handleGroupMessagesClear(ws, data) {
  if (!requireAuthenticated(ws)) return;

  const groupId = String(data.groupId || "");
  if (!groupId || !isGroupMember(groupId, ws.username)) {
    safeSend(ws, { type: "error", message: "Нет доступа к очистке этой группы" });
    return;
  }
...
function handleRegister(ws, data) {
  ...
  console.log("[SERVER] Register attempt:", { username, lastName, firstName, patronymic });
...
function handleClientMessage(ws, rawMessage) {
  ...
  break;
    ...
    case "groups:list":
      handleGroupsList(ws);
      break;
    case "group:messages:get":
      handleGroupMessagesGet(ws, data);
      break;
    case "group:message":
      handleGroupTextMessage(ws, data);
      break;
    case "group:file":
      handleGroupFileMessage(ws, data);
      break;
    case "group:messages:clear":
      handleGroupMessagesClear(ws, data);
      break;
...
server.listen(PORT, HOST, () => {
  ...
  console.log(`Group history file: ${GROUP_MESSAGES_FILE}`);
