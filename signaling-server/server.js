...
const MAX_GROUP_HISTORY_MESSAGES = Number(process.env.MAX_GROUP_HISTORY_MESSAGES || 500);

/**
 * Сервер предназначен для облачного запуска.
 * Для учебной демонстрации сервер хранит историю текстовых сообщений групп,
 * чтобы она была видна всем участникам при входе с разных аккаунтов.
 * Личные сообщения и файлы по-прежнему могут передаваться через WebRTC DataChannel.
 * Сервер отвечает за:
...
const server = http.createServer((req, res) => {
  ...
  if (url.pathname === "/" || url.pathname === "/health") {
    const payload = JSON.stringify({
      ...
      totalStoredGroupMessages: Object.values(groupMessages).reduce((sum, items) => sum + (Array.isArray(items) ? items.length : 0), 0),
...
let groupMessages = loadGroupMessages();
...
// ========== ЗАГРУЗКА/СОХРАНЕНИЕ ИСТОРИИ ГРУППОВЫХ СООБЩЕНИЙ ==========
...
function saveGroupMessages() {
  ...
  fs.writeFileSync(tmpFile, JSON.stringify(groupMessages, null, 2), "utf8");
  fs.renameSync(tmpFile, GROUP_MESSAGES_FILE);
}

function createGroupMessageId() {
  return `msg_${Date.now()}_${crypto.randomBytes(6).toString("hex")}`;
}

function getGroupHistory(groupId) {
  const history = Array.isArray(groupMessages[groupId]) ? groupMessages[groupId] : [];
  return history.slice(-MAX_GROUP_HISTORY_MESSAGES);
}

function appendGroupMessage(groupId, message) {
  if (!Array.isArray(groupMessages[groupId])) {
    groupMessages[groupId] = [];
  }
  groupMessages[groupId].push(message);
  if (groupMessages[groupId].length > MAX_GROUP_HISTORY_MESSAGES) {
    groupMessages[groupId] = groupMessages[groupId].slice(-MAX_GROUP_HISTORY_MESSAGES);
  }
  saveGroupMessages();
}

function sendGroupHistory(ws, groupId) {
  ...
  safeSend(ws, {
    type: "group:messages",
    groupId,
    messages: getGroupHistory(groupId),
  });
}

function broadcastGroupMessage(group, message) {
...
function handleGroupMessagesGet(ws, data) {
  if (!requireAuthenticated(ws)) return;
  sendGroupHistory(ws, String(data.groupId || ""));
}
...
function handleGroupMessage(ws, data) {
  ...
  if (!text) {
    safeSend(ws, { type: "error", message: "Нельзя отправить пустое сообщение" });
    return;
  }

  if (Buffer.byteLength(text, "utf8") > 4000) {
    safeSend(ws, { type: "error", message: "Сообщение слишком длинное" });
    return;
  }

  const message = {
    id: createGroupMessageId(),
    author: ws.username,
    text,
    timestamp: data.timestamp || new Date().toLocaleTimeString("ru-RU", { hour: "2-digit", minute: "2-digit", second: "2-digit" }),
    createdAt: nowIso(),
  };
